...
if [ $ROM == 3 ]; then
    # ZRAM TOGGLE
    if [ $ZRAM == 1 ]; then
	cp /tmp/emeriss.sh /system/system/vendor/etc/emeriss.sh
	chmod 755 /system/system/vendor/etc/emeriss.sh
    else
	cp /tmp/emeriss-zram.sh /system/system/vendor/etc/emeriss-zram.sh
	chmod 755 /system/system/vendor/etc/emeriss-zram.sh
    fi
    #
	cp /tmp/init.emeriss.rc /system/system/vendor/etc/init/hw/
	chmod 0644 /system/system/vendor/etc/init/hw/init.emeriss.rc
	...
	if [ $(grep -c "import /vendor/etc/init/hw/init.emeriss.rc" /system/system/vendor/etc/init/hw/init.qcom.rc) == 0 ]; then
		cp -p /system/system/vendor/etc/init/hw/init.qcom.rc /system/system/vendor/etc/init/hw/init.qcom.rc.bak
		sed -i "/import \/vendor\/etc\/init\/hw\/init\.target.rc/aimport /vendor/etc/init/hw/init.emeriss.rc" /system/system/vendor/etc/init/hw/init.qcom.rc
	fi
	##
...
#PIE AND LOWER RM SUPPORT
    ...
    # ZRAM TOGGLE
    if [ $ZRAM == 1 ]; then
	cp /tmp/emeriss.sh /system/etc/emeriss.sh
	chmod 755 /system/etc/emeriss.sh
    else
	cp /tmp/emeriss-zram.sh /system/etc/emeriss-zram.sh
	chmod 755 /system/etc/emeriss-zram.sh
    fi
    #
	cp -f /tmp/cpio /sbin/cpio
	cd /tmp/
	...
	cp /tmp/init.emeriss.rc /tmp/ramdisk/
	# ADD SPECTRUM SUPPORT
	...
	# COMPATIBILITY FIXES END
	chmod 0754 /tmp/ramdisk/init.emeriss.rc
	if [ $(grep -c "import /init.emeriss.rc" /tmp/ramdisk/init.rc) == 0 ]; then
	   sed -i "/import \/init\.\${ro.hardware}\.rc/aimport /init.emeriss.rc" /tmp/ramdisk/init.rc
	fi
